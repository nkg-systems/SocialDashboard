...
export const OAuthCallback: React.FC<OAuthCallbackProps> = ({
    ...
    const handleCallback = async () => {
      try {
        if (!socialService.isReturningFromOAuth()) {
          setStatus('error');
          setMessage('Invalid OAuth callback - no authorization code found');
          return;
        }
        ...
        if (callbackInfo.error) {
          setStatus('error');
          setMessage(`OAuth authorization failed: ${callbackInfo.error}`);
          socialService.handleOAuthError(callbackInfo.error);
          onError?.(callbackInfo.error);
          return;
        }

        if (!callbackInfo.code || !callbackInfo.platform) {
          setStatus('error');
          setMessage('Missing required OAuth parameters');
          return;
        }

        setPlatform(callbackInfo.platform);
        setMessage(`Completing ${callbackInfo.platform} connection...`);

        // The backend OAuth callback endpoint will handle the actual token exchange
        // Our frontend just needs to show a processing state and then redirect
        
        // Wait a moment for the backend to process the callback
        await new Promise(resolve => setTimeout(resolve, 2000));
        
        setStatus('success');
        setMessage(`Successfully connected to ${callbackInfo.platform}!`);
        
        socialService.handleOAuthSuccess(callbackInfo.platform);
        onSuccess?.(callbackInfo.platform);
        ...
        socialService.clearOAuthSession();
  ...
  const getPlatformIcon = (platformName: string) => {
    ...
    return iconMap[platformName.toLowerCase()] || (
